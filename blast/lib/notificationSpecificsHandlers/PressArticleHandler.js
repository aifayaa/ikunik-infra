...
PressArticleHandler.prototype.init = async function init() {
  const [article] = await this.client.db().collection(COLL_PRESS_ARTICLES).aggregate([
    { $match: {
  ...
  if (this.artCatBadges.length > 0) {
    const badgeIds = [];
    this.artCatBadges.forEach(({ list }) => { list.forEach(({ id }) => { badgeIds.push(id); }); });
...
PressArticleHandler.prototype.batchDone = async function batchDone(abort, retry) {
  if (!abort && !retry) {
    await this.client.db().collection(COLL_PRESS_ARTICLES).updateOne(
      { _id: this.queueData.articleId },
