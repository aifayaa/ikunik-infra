export default (libs, output) => {
  const method = libs.make.method('Edits a press category', [
  ...
  method.parameters = [
    libs.make.param(
      ...
      'string',
      true,
      'The article category ID to edit',
    ),
  ];
  ...
  method.requestBody = libs.make.requestBody('This is the whole request body', true, libs.make.schemaObject({
    ...
    parentId: libs.make.outParam('The parent category ID', 'string', false),
  ...
  method.responses = {
    200: libs.make.response(
      'Success',
      libs.make.outParam('True if the update was successful', 'boolean', true),
    ),
