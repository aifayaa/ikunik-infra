...
const doPipeline = (userId, {
  ...
  const pipeline = pipelineStart(userId);
...
const doSeach = async (pipeline, { page, limit = 20 }) => {
  ...
  try {
    ...
    const [crowd, fancount] = await Promise.all([
      client.db(process.env.DB_NAME).collection(process.env.COLL_NAME).aggregate(pipeline)
        .toArray(),
      client.db(process.env.DB_NAME).collection(process.env.COLL_NAME).aggregate(countPipeline)
        .toArray(),
    ]);
