  ...
  output.components.schemas.users = libs.make.schemaObject(
    {
      ...
      country: libs.make.outParam('The user country code', 'string', false, {
        example: 'CA',
      }),
      locale: libs.make.outParam('The user locale', 'string', false, {
        example: 'fr-CA',
      }),
      createdAt: libs.make.outParam('The user creation date', 'string', true, {
        example: '1970-12-31T23:59:59.000Z',
      }),
      emails: libs.make.schemaArray(
        libs.make.schemaObject({
          address: libs.make.outParam(
            'The user email address used for login',
            'string',
            true
          ),
          verified: libs.make.outParam(
            'Whether this email address was verified',
            'boolean',
            true
          ),
        })
    ...
    {
      description:
        'This schema lacks voluntarly some information, since they are never shared using the API',
    }
  );
};
