...
export default (libs, output) => {
  const method = libs.make.method(
    'Creates a new UGC (User Generated Content)',
  ...
  method.requestBody = libs.make.requestBody(
    ...
    libs.make.schemaObject({
      parentId: libs.make.outParam(
        'The previous UGC ID (for a reply)',
        'string',
        true
      ),
      ...
      rootParentId: libs.make.outParam(
        'The root parent ID (article or UGC)',
        'string',
        false
      ),
      rootParentCollection: libs.make.outParam(
        'The root parent collection on which the UGC was made',
        'string',
        false
      ),
    })
  ...
  method.responses = {
    200: libs.make.response(
      'Success',
      libs.make.schemaRef('schemas', 'userGeneratedContents')
    ),
