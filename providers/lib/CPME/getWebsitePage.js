...
const { CPME_PATH_AUTH, CPME_URL, CPME_CRED_PASS, CPME_CRED_USER } =
  process.env;

const allowedPaths = ['/annuaire/institutions', '/annuaire/adherents'];
...
export default async (pagePath) => {
  ...
  cookiesJar.setCookie(
    requestLib.cookie('utilisation_cookies=close'),
    CPME_URL
  );
  ...
  const authPath = CPME_PATH_AUTH.replace(
    'RETURN_PATH_BASE64',
    returnPathBase64
  );
...
div.span8 {
  ...
  box-sizing: border-box !important;
    ...
    .replace(
      /<link( [^>]*)? href=(["'])\/([^/])/g,
      `<link$1 href=$2${CPME_URL}/$3`
    )
    .replace(
      /<script( [^>]*)? src=(["'])\/([^/])/g,
      `<script$1 src=$2${CPME_URL}/$3`
    );
};
