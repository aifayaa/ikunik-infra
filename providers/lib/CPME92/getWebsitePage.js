...
div.Content-outer {
  padding-top: 30px;
    ...
    .replace(/<a( [^>]*)? href='(\/[^']*)'/g, '<a$1 href=\'#\'')
    .replace(/<img( [^>]*)? src=(["'])\/([^/])/g, `<img$1 src=$2${CPME92_URL}/$3`)
    .replace(/<link( [^>]*)? href=(["'])\/([^/])/g, `<link$1 href=$2${CPME92_URL}/$3`)
    .replace(/<script( [^>]*)? src=(["'])\/([^/])/g, `<script$1 src=$2${CPME92_URL}/$3`)
  );
};
