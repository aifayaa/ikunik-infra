...
export default (libs, output) => {
  const method = libs.make.method(
    'Returns public informations about your user',
    [{ name: 'users' }]
  );
  ...
  method.parameters = [
    libs.make.param(
      ...
      'string',
      true,
      'The user to return. Must be your own user ID.'
    ),
  ];
    ...
    200: libs.make.response(
      ...
      libs.make.schemaObject({
        ...
        emails: libs.make.schemaArray(
          libs.make.outParam('Email address', 'string', true, {
            example: 'test@te.st',
          }),
          { description: 'An array of email addresses for this user' }
        ),
        hasArtistProfile: libs.make.outParam(
          'Outdated property',
          'boolean',
          false
        ),
        locale: libs.make.outParam('The locale of this user', 'string', true, {
          example: 'fr-CA',
        }),
        profile: libs.make.schemaObject(
          {
            ...
            email: libs.make.outParam('Email address', 'string', false, {
              example: 'test@te.st',
            }),
            first_name: libs.make.outParam(
              'The user first name',
              'string',
              false
            ),
            last_name: libs.make.outParam(
              'The user last name',
              'string',
              false
            ),
          ...
          {
            description:
              'User profile data object. This schema is not complete and most fields are optionnals',
          }
        ),
        username: libs.make.outParam('The username', 'string', true),
