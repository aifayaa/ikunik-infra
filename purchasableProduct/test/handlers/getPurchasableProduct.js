...
describe('handlers - getPurchasableProduct', () => {
  let stubLib;
  const event = {
    requestContext: {
      authorizer: {
        appId: 'crowdaa_app_id',
        perms: JSON.stringify({ purchasableProduct_get: true }),
      },
    },
    pathParameters: {
      id: 'productId',
  ...
  describe('lib error', () => {
    ...
    describe('lib fail', () => {
      ...
      before(() => {
        stubLib = sandbox.stub(lib, 'getPurchasableProduct').callsFake(() => Promise.reject(libResult));
      });
