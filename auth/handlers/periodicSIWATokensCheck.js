  ...
  try {
    ...
    if (results.length > 0) {
      ...
      if (rejected.length > 0) {
        ...
        const list = rejected.map((result) => (formatMessage('auth:siwa_periodic_renewal.list_line_error', {
          app: result.app,
      ...
      if (fulfilled.length > 0) {
        ...
        const list = fulfilled.map((result) => (formatMessage('auth:siwa_periodic_renewal.list_line_success', {
          app: result.app,
        }))).join('</li>\n<li>');
        toMail += `<ul><li>${list}</li></ul>`;
      }
