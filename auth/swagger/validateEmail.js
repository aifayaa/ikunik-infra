...
export default (libs, output) => {
  const handler = {
    post: libs.make.method(
      'Marks an email address of an account as validated in database',
      [{ name: 'auth' }]
    ),
  };
  ...
  handler.post.requestBody = libs.make.requestBody(
    ...
    libs.make.schemaObject({
      email: libs.make.outParam(
        'The email of the account to validate',
        'string',
        true
      ),
  ...
  handler.post.responses = {
    200: libs.make.response(
      'Success',
      libs.make.schemaObject({
        status: libs.make.outParam('The "success" string', 'string', true, {
          example: 'success',
        }),
      })
    ),
