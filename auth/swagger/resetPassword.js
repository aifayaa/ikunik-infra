...
export default (libs, output) => {
  const handler = {
    post: libs.make.method(
      'Resets a user password, providing a token sent by email and and a new password',
      [{ name: 'auth' }]
    ),
  };
  ...
  handler.post.requestBody = libs.make.requestBody(
    ...
    libs.make.schemaObject({
      email: libs.make.outParam(
        'The email of the account to reset password',
        'string',
        true
      ),
      token: libs.make.outParam(
        'The token that was sent by email',
        'string',
        true
      ),
      password: libs.make.outParam(
        'The new password of this account',
        'string',
        true
      ),
    })
  ...
  handler.post.responses = {
    200: libs.make.response(
      ...
      libs.make.schemaObject({
        ...
        message: libs.make.outParam('The "ok" string', 'string', true, {
          example: 'ok',
        }),
      })
    ),
