...
export const changePassword = async (userId, oldPassword, password, appId, lang) => {
  ...
  try {
    ...
    const user = await usersCollection.findOne(
      ...
      {
        projection: { _id: true, emails: true, 'services.password.bcrypt': true, 'profile.username': true },
      },
    );
    ...
    if (!get(user, 'services.password.bcrypt')) throw new Error('user_not_found');
    ...
    const html = formatMessage('auth:password_changed_email.html', { username: user.profile.username, email });
