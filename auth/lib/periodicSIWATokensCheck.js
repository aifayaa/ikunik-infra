...
async function refreshJwtToken(
  ...
  let s3KeyResponse;
  try {
    s3KeyResponse = await s3.getObject({
  ...
  try {
    ...
    throw new Error(`Error parsing/loading key (stored at S3 path ${s3Path}) : ${e}`);
  }
      ...
      .updateOne(
        { _id: app._id },
        {
          $set: {
            ...
            'credentials.apple.expireTime': parseInt((Date.now() / 1000) + (validity * 86400), 10),
          },
        },
      );
  } finally {
...
export const periodicSIWATokensCheck = async () => {
  ...
  try {
    const expireMinTime = parseInt((Date.now() / 1000) + (DEFAULTS.delay * 86400), 10);
    const toExpireApps = await client.db().collection(COLL_APPS).find({
