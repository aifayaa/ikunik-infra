export default (libs, output) => {
  ...
  post.parameters = [
    libs.make.param('autoPublish', 'query', 'boolean', false, 'Publish this article now'),
    // libs.make.param('forceCategoryId', 'query', 'string', false,
    //     'Overrides the "categoryId" parameter from the body'),
    // libs.make.param('forcePictures', 'query', 'string', false,
    //     'Overrides the "pictures" parameter from the body'),
    ...
    libs.make.paramBody('app', 'Parameters', true, libs.make.schemaObject({
      ...
      pictures: libs.make.outParam('An array of pictures IDs', 'string', false),
      ...
      videos: libs.make.outParam('An array of videos IDs', 'string', false),
    })),
  ];

  post.responses = {
    200: libs.make.response('Success', libs.make.schemaObject({
      articleId: libs.make.outParam('The newly created article ID', 'string', true, { example: 'b5dcc350-1052-4349-a271-859e44e2f80c' }),
      draftId: libs.make.outParam('The newly created draft ID', 'string', true, { example: 'b5dcc350-1052-4349-a271-859e44e2f80c' }),
      productId: libs.make.outParam('The reference of the product, only used for In App Purchase feature', 'string', false, { example: 'b5dcc350-1052-4349-a271-859e44e2f80c' }),
      published: libs.make.outParam('Set to true when the article was published', 'boolean', false),
      notificationSent: libs.make.outParam('Set to true when the notification was sent', 'boolean', false),
    })),
