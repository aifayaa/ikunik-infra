export default (libs, output) => {
  ...
  handler.post.parameters = [
    libs.make.param('category', 'body', 'string', true, 'The category ID to look into'),
    libs.make.param('start', 'body', 'string', true, 'The offset for the results'),
    libs.make.param('limit', 'body', 'string', true, 'The number of results to return'),
  ];
    ...
    200: libs.make.response('Success', libs.make.schemaObject({
      articles: libs.make.schemaArray(libs.make.schemaObject({
        ...
        category: libs.make.schemaObject({
          ...
          appIds: libs.make.schemaRef('fieldAppIds'),
        ...
        pictures: libs.make.schemaArray(libs.make.schemaRef('collPictures')),
        ...
        text: libs.make.outParam('The article title', 'string', true, { example: 'My article' }),
        title: libs.make.outParam('The article content, as HTML', 'string', true, { example: '<p>example</p>' }),
        videos: libs.make.outParam('An array of videos, if any', 'array', false, { example: [] }),
      }), { description: 'An array of all articles found according to input parameters' }),
    })),
