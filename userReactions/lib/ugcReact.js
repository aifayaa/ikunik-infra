...
const { COLL_PRESS_ARTICLES, COLL_USERS, COLL_USER_GENERATED_CONTENTS } =
  mongoCollections;
...
export default async function ugcReact(
  appId,
  ugcId,
  userId,
  reaction,
  ...
  try {
    ...
    if (insertedReaction) {
      const [rootUgc, fromUser] = await Promise.all([
        client.db().collection(COLL_USER_GENERATED_CONTENTS).findOne({
          _id: ugcId,
          appId,
        }),
      ...
      if (rootUgc && rootUgc.userId !== userId) {
        ...
        if (!rootUgc.parentCollection) {
          await sendNotificationFor(
            {
              appId,
              content: formatMessage('ugc:ugc_post_reacted_push.text', {
          ...
          await sendNotificationFor(
            {
              appId,
              content: formatMessage('ugc:ugc_comment_reacted_push.text', {
                username: fromUser.profile.username,
              }),
              extraData,
