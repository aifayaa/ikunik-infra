export default (libs, output) => {
  const method = libs.make.method('Returns all supported file formats', [{ name: 'files' }]);

  method.responses = {
    200: libs.make.response('Success', libs.make.schemaObject({}, {
      description: 'An object where each key is a mime type, and all values are booleans, to indicate support',
      example: {
        'image/gif': true,
        'image/jpeg': true,
        'image/png': true,
